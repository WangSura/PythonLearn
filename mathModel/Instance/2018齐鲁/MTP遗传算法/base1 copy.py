# https://blog.csdn.net/weixin_42201701/article/details/103746595?utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7EBlogCommendFromMachineLearnPai2%7Edefault-18.control&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7EBlogCommendFromMachineLearnPai2%7Edefault-18.control
import pandas as pd
import numpy as np
import random
import copy
...
def xy():
    ...
    x = pd.read_excel(
        "D:/program/pythonPractice/mathModel/Instance/2018齐鲁/暑假第一阶段/相关的要素名称及位置坐标数据.xls", usecols=[1])
    y = pd.read_excel(
        "D:/program/pythonPractice/mathModel/Instance/2018齐鲁/暑假第一阶段/相关的要素名称及位置坐标数据.xls", usecols=[2])
    #x = pd.DataFrame(x)
    #x = x.values
    #y = pd.DataFrame(y)
    #y = y.values
    ...
    #point_name = pd.DataFrame(point_name)
    #li = point_name.values
    #li = np.array(li)
    #np.savetxt('li', x, y)
    #li = pd.DataFrame(li)
    # li.to_excel(
    #   "D:/program/pythonPractice/mathModel/Instance/2018齐鲁/暑假第一阶段/li.xls")
    # return np.array(
...
def fitness(population, DMAT, break_points, aimFunction):
    ...
    value = []
    for i in range(len(population)):
        value.append(aimFunction(
            population[i], DMAT, copy.deepcopy(break_points)))
    ...
    for i in range(half):
        if np.random.uniform(0, 1) <= pc:
            ...
            if cut1 == cut2:
                ...
                son = father[i][0:cut1] + \
                    mother[i][cut1:cut2] + father[i][cut2:]
                son = amend(son, cut1, cut2)
                daughter = mother[i][0:cut1] + \
                    father[i][cut1:cut2] + mother[i][cut2:]
...
if __name__ == '__main__':
    ...
    for i in range(4001):
        ...
        for j in range(len(population)):
            result.append(
                1.0 / aimFunction(population[j], DMAT, copy.deepcopy(break_points)))
