...
class jgraph(object):
    def __init__(self, matrix):
        ...
        self.file_path = os.path.join(os.path.expanduser(
            "~"), 'Desktop') + '\\social_network.png'
        self.shortest_file_path = os.path.join(os.path.expanduser(
            "~"), 'Desktop') + '\\shortest_paths_social_network.png'
        self.g = igraph.Graph(directed=self.__directed)
        self.visual_style = {"vertex_size": 10, "bbox": (
            800, 800), 'margin': 20, 'vertex_label_size': 15, 'edge_arrow_size': 0.5, 'vertex_label_dist': 2}
    ...
    def __check(self):
        for i in range(self.shape[0]):
            if self.matrix.iloc[i][i] != 0:
                warnings.warn(
                    "Diagonal elements don't make sense, program has been setting it to 0")
    ...
    def shortest_paths_without_weights_id(self, begin, end):
        '''通过id获取不带权最短路径'''
        shortest_paths = self.g.get_all_shortest_paths(
            self.g.vs[begin], self.g.vs[end])  # 储存最短路径的列表
        edges_id = self.__get_eid_from_list(shortest_paths)
        shortest_visual_style = self.visual_style.copy()
        shortest_visual_style["edge_width"] = [
            4 if i in edges_id else 1 for i in range(len(self.g.es))]
        igraph.plot(self.g, self.shortest_file_path, **shortest_visual_style)
        for (d, shortest_path) in enumerate(shortest_paths):
            shortest_path_list = [self.matrix.index[shortest_path[i]]
    ...
    def shortest_paths_id(self, begin, end):
        '''通过 id 获取最短路径'''
        shortest_paths = self.g.get_all_shortest_paths(
            self.g.vs[begin], self.g.vs[end], weights=self.g.es['weight'])  # 储存最短路径的列表
        edges_id = self.__get_eid_from_list(shortest_paths)
        shortest_visual_style = self.visual_style.copy()
        shortest_visual_style["edge_width"] = [
            4 if i in edges_id else 1 for i in range(len(self.g.es))]
