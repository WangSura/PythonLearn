...
cost_matrix = pd.read_excel(
    "D:/program/pythonPractice/数模/2018齐鲁/暑假第一阶段/距离矩阵.xlsx", index_col=0)
...
MODEL.addConstrs(y[z] == 1 for z in [
                 point for point in cost_matrix.index if point.startswith("Z")])
MODEL.addConstrs(gamma[z] == sum(b[z, j] * cost_matrix.at[z, j]
...
if __name__ == '__main__':
    from data3_shortest import cost_matrix, A_graph, B_graph, graph
    import pygraphviz as pgv

    G = pgv.AGraph(directed=False, concentrate=True)
    colors = dict(zip(clusters.keys(), [
                  "#60acfc", "#32d3eb", "#5bc49f", "#feb64d", "#ff7c7c", "#9287e7", "#999999", "#F781FF"]))
    colors.update({end: colors[start]
                  for start, ends in clusters.items() for end in ends})

    # 添加节点
    for point in point_location.index:
        color = colors[point] if point in colors.keys() else "#5bc49f32" if point.startswith(
            "D") else "#0000ff" if point.startswith("F") else "#ff000032"
    ...
    for start, ends in clusters.items():
        for paths in graph.shortest_paths([start], ends, "multi", False):
            G.add_edges_from(
                list(zip(paths[0][0], paths[0][0][1:])), color=colors[start], penwidth=3)

    # 导出图形
    G.layout()
    G.draw("D:/program/pythonPractice/数模/2018齐鲁/暑假第一阶段/p4聚类结果.png")
