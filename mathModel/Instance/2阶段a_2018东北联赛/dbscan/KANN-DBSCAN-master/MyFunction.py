...
def CalculateDistMatrix(dataset):
    ...
    :return: 距离矩阵
    """
    DistMatrix = [[0 for j in range(len(dataset))]
...
def returnClusterNumberList(dataset, EpsCandidate, MinptsCandidate):
    ...
    for i in range(len(EpsCandidate)):
        clustering = DBSCAN(
            eps=EpsCandidate[i], min_samples=MinptsCandidate[i]).fit(np_dataset)
...
if __name__ == '__main__':
    dataSet = loadDataSet(
        'D:/program/pythonPractice/mathModel/Instance/2阶段a_2018东北联赛/二阶段/漏损速率参数.txt', splitChar=',')
    EpsCandidate = returnEpsCandidate(dataSet)
    DistMatrix = CalculateDistMatrix(dataSet)
    MinptsCandidate = returnMinptsCandidate(DistMatrix, EpsCandidate)
    ClusterNumberList = returnClusterNumberList(
        dataSet, EpsCandidate, MinptsCandidate)
    print(EpsCandidate)
    EpsCandidate = pd.DataFrame(EpsCandidate)
    EpsCandidate.to_excel(
        'D:/program/pythonPractice/mathModel/Instance/2阶段a_2018东北联赛/二阶段/EpsCandidate2.xlsx')
    print(MinptsCandidate)
    MinptsCandidate = pd.DataFrame(MinptsCandidate)
    MinptsCandidate.to_excel(
        'D:/program/pythonPractice/mathModel/Instance/2阶段a_2018东北联赛/二阶段/MinptsCandidate2.xlsx')
    print('cluster number list is')
    ClusterNumberList = pd.DataFrame(ClusterNumberList)
    ClusterNumberList.to_excel(
        'D:/program/pythonPractice/mathModel/Instance/2阶段a_2018东北联赛/二阶段/ClusterNumberList2.xlsx')
