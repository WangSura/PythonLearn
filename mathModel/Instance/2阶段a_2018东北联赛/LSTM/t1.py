...
if __name__ == '__main__':
    ...
    lstm_model = LstmRNN(INPUT_FEATURES_NUM, 16,
                         output_size=OUTPUT_FEATURES_NUM, num_layers=1)  # 16 hidden units
    ...
    predictive_y_for_training = predictive_y_for_training.view(
        -1, OUTPUT_FEATURES_NUM).data.numpy()
    ...
    plt.plot(t_for_testing, predictive_y_for_testing,
             'm--', label='pre_cos_tst')

    plt.plot([t[train_data_len], t[train_data_len]], [-1.2, 4.0],
             'r--', label='separation line')  # separation line
