...
posi = pd.read_excel(
    r'D:/program/pythonPractice/mathModel/Instance/2阶段a_2018东北联赛/二阶段/p3_version1/A题原数据分类后.xls')
lon = np.array(posi["P"][:])  # 经度
B = list(zip(lon, np.zeros_like(lon)))  # 按照纬度-经度匹配

# 获取会员对任务密度，取第5邻域，阈值为 100，权重分别为5，1
outliers4, inliers4 = ensemble_lof(
    A, predict=None, k=23, groups=[[0, 1]], method=1, vote_method='auto')
outliers4.to_excel(
    "D:/program/pythonPractice/mathModel/Instance/2阶段a_2018东北联赛/二阶段/p3_version1/out1.xlsx")
inliers4.to_excel(
    "D:/program/pythonPractice/mathModel/Instance/2阶段a_2018东北联赛/二阶段/p3_version1/in1.xlsx")

# 获取会员对任务密度，取第5邻域，阈值分别为 1.5，2，得票数超过 1 的认为是异常点


# 绘图程序
c = pd.concat([inliers4, outliers4])
plt.figure('Voting integration LOF pattern')
#plt.scatter(np.array(A)[:, 0], np.array(A)[:, 1], s=10, c='b', alpha=0.5)
plt.scatter(np.array(B)[:, 0], np.array(A)[:, 0], s=10, c='green', alpha=0.8)
#plt.scatter(np.array(A)[:, 0], np.array(B)[:, 0], s=10, c='b', alpha=0.8)
plt.scatter(np.array(B)[:, 0], c[0],
            s=c["vote"]*(10+1000), c='r', alpha=0.2)
